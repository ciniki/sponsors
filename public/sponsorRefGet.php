...
function ciniki_sponsors_sponsorRefGet($ciniki) {
    ...
    $rc = ciniki_sponsors_checkAccess($ciniki, $args['tnid'], 'ciniki.sponsors.sponsorRefGet'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    $strsql = "SELECT ciniki_sponsor_objrefs.id AS ref_id, "
        ...
        . "LEFT JOIN ciniki_sponsors ON ( "
            ...
            . "AND ciniki_sponsor_objrefs.tnid = '" . ciniki_core_dbQuote($ciniki, $args['tnid']) . "' "
            . ") "
    ...
    if( !isset($rc['sponsors']) || !isset($rc['sponsors'][0]) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sponsors.19', 'msg'=>'Unable to find sponsor'));
    }
