...
function ciniki_sponsors_sponsorRefAdd(&$ciniki) {
    ...
    $rc = ciniki_sponsors_checkAccess($ciniki, $args['tnid'], 'ciniki.sponsors.sponsorRefAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    //
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.sponsors.objref', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
